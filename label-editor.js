...
function getCurrentSubjects(student) {
    return getSubjectsArray(student).filter(subject => subject?.status === 'current');
}
...
function getLabelCountForStudent(student) {
    return getCurrentSubjects(student).length;
}

/**
 * ✅ DUPLICATE LOGIC:
 * Creates one identical label entry per current subject.
 * Example: Student with 2 current subjects gets 2 entries.
 */
function duplicateStudentsBySubjectCount(students) {
    return students.flatMap(student => {
        const labelCount = getLabelCountForStudent(student);
        if (labelCount <= 0) return [];
        return Array.from({ length: labelCount }, () => student);
    });
}
...
async function initApp() {
    ...
    try {
        ...
        exportDocxBtn.disabled = true;
        if (studentCountEl) {
            studentCountEl.textContent = '❌ Unable to load students. Please refresh the page and try again.';
        }
        return;
    }
    ...
    const totalLabelCount = duplicateStudentsBySubjectCount(allStudents).length;
    
    if (studentCountEl) {
        studentCountEl.textContent = allStudents.length
            ? `Loaded ${allStudents.length} active students (${totalLabelCount} name cards). Select a grade and generate preview.`
    ...
    generateBtn.addEventListener('click', () => {
        ...
        // ✅ Apply Duplication for Preview
        const labelEntries = duplicateStudentsBySubjectCount(filtered);
        
        studentCountEl.textContent = `Found ${filtered.length} active students (${labelEntries.length} name cards).`;
        
        printArea.innerHTML = '';
        const chunkSize = 24;
        
        for (let i = 0; i < labelEntries.length; i += chunkSize) {
            const chunk = labelEntries.slice(i, i + chunkSize);
        ...
        exportDocxBtn.disabled = labelEntries.length === 0;
    ...
    exportDocxBtn.addEventListener('click', () => {
        ...
        // ✅ Apply Duplication for Export
        const labelEntries = duplicateStudentsBySubjectCount(filtered);
        exportToDOCX(labelEntries);
    });
}
...
function exportToDOCX(students) {
    ...
    students.forEach(student => {
        const subjects = getSubjectsArray(student);
    ...
    return '<div class="label-cell">' +
        '<div class="label-left">' +
            '<div class="info-row">' +
                ...
                '<span class="info-value">' + escapeHtml(chineseName) + '</span>' +
            '</div>' +
            '<div class="info-row">' +
                '<span class="info-label">NAME:</span>' +
                '<span class="info-value">' + escapeHtml(pinyinName) + '</span>' +
            '</div>' +
            '<div class="info-row">' +
            ...
            '<div class="info-row">' +
                ...
                '<span class="info-value">' + escapeHtml(studentNo) + '</span>' +
            '</div>' +
        '</div>' +
        '<div class="label-right">' +
            '<div class="unified-grid">' +
                ...
                '<div>' + escapeHtml(enrolChinese) + '</div>' +
                '<div>' + escapeHtml(enrolEnglish) + '</div>' +
                '<div>' + escapeHtml(enrolEFL) + '</div>' +
                '<div>' + escapeHtml(enrolMath) + '</div>' +
