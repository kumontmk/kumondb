...
let currentStudentData = null; // Global store for AT table rendering
...
// NEW: Collect DT Table Data (Including Suggested/Actual Start)
...
if (!container) return;
// ✅ SMART PARSER: If report says "ZI11", split it into level="ZI" and ws="11"
if (data.pencilSkill) {
let rawLevel = String(data.pencilSkill.level || '');
let rawWs = String(data.pencilSkill.ws || '');
const match = rawLevel.match(/^(ZI|ZII)(\d+)$/i);
if (match) {
data.pencilSkill.level = match[1].toUpperCase(); // "ZI" or "ZII"
data.pencilSkill.ws = match[2];                  // "11", "5", etc.
}
}
...
function addDTRow(data = {}) {
  ...
  const tr = document.createElement('tr');
  tr.innerHTML = `
    <td><select class="dt-subject" required style="width:100%; padding:0.5rem;">
      <option value="">Select Subject</option>
    ...
    progArray.forEach(prog => {
      if (prog.test && prog.test.date) {
        ...
        tr.innerHTML = `
          ...
          <td>${prog.test.level || '-'}</td>
          <td>${prog.test.date}</td>
          <td>${prog.test.score || '-'}</td>
          <td>${prog.test.time || '-'}</td>
          <td>${prog.test.group || '-'}</td>
        `;
        tbody.appendChild(tr);
      }
    });
...
if (snap.exists()) {
let s = snap.val();
currentStudentData = s; // Store globally for AT rendering
