...
function addCard(container, employee, inLog, outLog) {
  ...
  const initial = (employee.englishName || employee.chineseName || '?').charAt(0).toUpperCase();
  const outTime = outLog ? outLog.time : t('timecard.na');
  const outClass = outLog ? '' : 'na';
  const location = (inLog && inLog.location && inLog.location !== '-')
    ? inLog.location
    : (outLog && outLog.location && outLog.location !== '-' ? outLog.location : '-');

  card.innerHTML = `
    <div class="tc-header">
      <div class="tc-avatar">${initial}</div>
      <div class="tc-info">
        <div class="tc-name-cn">${employee.chineseName}</div>
        <div class="tc-name-en">${employee.englishName}</div>
        <div class="tc-position">${employee.position || t('timecard.employee')}</div>
      </div>
    </div>
    <div class="tc-times">
      <div class="tc-time-box in">
        <span class="label">${t('timecard.in')}</span>
        <span class="time">${inLog ? inLog.time : '-'}</span>
      </div>
      <div class="tc-time-box out">
        <span class="label">${t('timecard.out')}</span>
        <span class="time ${outClass}">${outTime}</span>
      </div>
    </div>
...
if (startNfcBtn) {
  startNfcBtn.addEventListener('click', async () => {
    if (!('NDEFReader' in window)) {
      showResultModal(false, t('timecard.nfcNotSupported'));
      return;
    }
    if (!currentEmployeeId) {
      showResultModal(false, t('timecard.nfcLoginRequired'));
      return;
    }
    ...
    try {
      ...
      if (err.name === 'NotAllowedError') {
        showResultModal(false, t('timecard.nfcPermissionDenied'));
      } else {
        showResultModal(false, `${t('timecard.nfcError')}${err.message}`);
      }
    }
  });
}
  ...
  if (centerId) {
    ...
    const checkInterval = setInterval(() => {
      ...
      if (employeesLoaded && centersLoaded && authInitialized) {
        ...
        if (centerData) saveAttendance(currentEmployeeId, centerData.name);
        else showResultModal(false, '❌ Center ID not found in database.');
      }
    }, 200);
...
async function handleUploadedQr(file) {
  ...
  showResultModal(true, t('timecard.readingUploadedImage'));
  try {
...
async function startCameraScan() {
  ...
  try {
    ...
    if (started) {
      isScanning = true;
      startScanBtn.textContent = t('timecard.scanningActive');
      startScanBtn.disabled = true;
    }
  } catch (err) {
    console.error('Scanner init failed:', err);
    await closeScanner();
    showResultModal(false, `${t('timecard.cameraError')}${err.message}`);
  }
}
...
async function handleScanSuccess(decodedText) {
  ...
  if (Object.keys(employees).length === 0) {
    await closeScanner();
    showResultModal(false, t('timecard.employeeDatabaseLoading'));
    return;
  }
  let matchedKey = null;
  let matchedEmp = null;
...
async function processAttendance(empId, emp) {
  if (!navigator.geolocation) { showResultModal(false, t('timecard.geolocationNotSupported')); return; }
  try {
    ...
    if (!matchedCenter) {
      ...
      showResultModal(false, `${t('timecard.outsideRange')}${distances.sort((a, b) => a.dist - b.dist)[0].name} (${distances[0].dist.toFixed(3)}${t('timecard.km')})`);
      return;
    }
    await saveAttendance(empId, matchedCenter.name);
  } catch (err) {
    const messages = {
      1: t('timecard.locationPermissionDenied'),
      2: t('timecard.locationUnavailable'),
      3: t('timecard.locationTimeout')
    };
    showResultModal(false, messages[err.code] || `${t('timecard.locationError')}${err.message}`);
  }
}
  ...
  try {
    ...
    if (!lastLog || lastLog.type === 'out') {
      ...
      if (empTerms === 'Full-time') {
        ...
        autoOutLog = { type: 'out', time: minutesToTime(autoOutMins), location: lastLog.location, autoGenerated: true };
      }
    } else {
      nextType = 'out';
    }
    ...
    if (autoOutLog) msg += `${t('timecard.autoClockedOut')}${autoOutLog.location} (${autoOutLog.time})`;
...
if (exportCsvBtn) {
  exportCsvBtn.addEventListener('click', () => {
    ...
    if (Object.keys(currentDayLogs).length === 0) {
      alert(t('timecard.noAttendanceData'));
      return;
    }
