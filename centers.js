...
async function isManagerUser(user, userData) {
  ...
  const positions = getEmpPositions(userData).map(p => String(p).trim().toLowerCase());
  ...
  try {
    ...
    if (empUidSnap.exists()) {
      ...
      if (emp?.isDisabled !== true) {
        const empPositions = getEmpPositions(emp).map(p => String(p).trim().toLowerCase());
    ...
    if (email) {
      ...
      if (empSnap.exists()) {
        const matchingEmp = Object.values(empSnap.val()).find(e =>
          normalizeText(e.email) === email
        );

        if (matchingEmp && matchingEmp.isDisabled !== true) {
          const empPositions = getEmpPositions(matchingEmp).map(p => String(p).trim().toLowerCase());
  ...
  try {
    ...
    if (!centersSnap.exists()) {
      centerGrid.innerHTML = `
        <p style="text-align:center; color:#666; grid-column: 1/-1;">
          No centers found in database. Please contact admin.
        </p>
      `;
    ...
    centerGrid.innerHTML = `
      <p style="text-align:center; color:#dc3545; grid-column: 1/-1;">
        Error loading centers. Please refresh.
      </p>
    `;
      ...
      modal.innerHTML = `
        <div class="modal-content" style="max-width: 600px; text-align: left;">
          ...
          <h3 style="text-align: center; color: #dc3545; margin-bottom: 1rem;">
            ⚠️ Missing Clock-Out Records
          </h3>

          <p style="color: #666; margin-bottom: 1.5rem; text-align: center;">
            You have incomplete timecard records from previous days.
            Please provide the missing times for manager approval.
          </p>

          <div id="missingRecordsList" style="max-height: 400px; overflow-y: auto; margin-bottom: 1rem;"></div>

          <div style="display:flex; gap:1rem; justify-content:flex-end;">
            <button class="secondary" id="remindLaterBtn" type="button">Remind Me Later</button>
            <button class="primary" id="submitMissingClockOutsBtn" type="button">Submit for Review</button>
          </div>
        </div>
      `;
    ...
    records.forEach(rec => {
      ...
      item.innerHTML = `
        <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:0.5rem;">
          <span style="font-weight:600; color:#4682B4;">📅 ${escapeHtml(rec.date)}</span>
          <span style="font-size:0.85rem; color:#666;">📍 ${escapeHtml(rec.center || 'Unknown')}</span>
        </div>

        <div style="font-size:0.9rem; margin-bottom:0.5rem;">
          Clock-In: <strong>${escapeHtml(rec.inTime)}</strong> |
...
async function checkPendingApprovals({
  ...
  try {
    ...
    const pending = Object.entries(verifications)
      ...
      .filter(v => String(v.missingType || '').toLowerCase() === 'out' && v.proposedOutTime)
      .filter(v => {
        // Admin sees all
...
function showPendingApprovalsModal(records) {
  ...
  modal.innerHTML = `
    <div class="modal-content pending-approvals-modal">
      <button class="close-btn pending-close-btn" type="button">&times;</button>
      ...
      <p style="color:#666; margin-bottom:1rem; text-align:center;">
        Employees have submitted proposed clock-out times. Review and approve or deny them below.
      </p>

      <div class="pending-summary">
        <span id="pendingApprovalCount" class="pending-count"></span>
        <span class="pending-hint">Select rows to approve/deny multiple requests.</span>
      </div>
      ...
      <div class="pending-table-wrapper">
        <table class="pending-approvals-table">
          <thead>
            <tr>
              <th><input type="checkbox" id="selectAllPending" class="pending-checkbox"></th>
              <th>Employee</th>
              <th>Date</th>
              <th>Center</th>
              <th>Clock In</th>
      ...
      <div class="pending-modal-actions">
        <button class="secondary pending-close-btn" type="button">Close</button>
        <button class="danger" id="denySelectedPendingBtn" type="button" disabled>Deny Selected</button>
        <button class="primary" id="approveSelectedPendingBtn" type="button" disabled>Approve Selected</button>
      </div>
    </div>
  `;
  ...
  function updateCount() {
    ...
    countEl.textContent = `${remaining} pending approval${remaining === 1 ? '' : 's'}`;
  ...
  function getSelectedIds() {
    return Array.from(modal.querySelectorAll('.pending-select:checked')).map(cb => cb.value);
  }
  ...
  function updateBulkButtons() {
    ...
    const boxes = Array.from(tbody.querySelectorAll('.pending-select:not(:disabled)'));

    selectAll.checked = boxes.length > 0 && boxes.every(cb => cb.checked);
    selectAll.indeterminate = boxes.some(cb => cb.checked) && !boxes.every(cb => cb.checked);
  }
  ...
  records.forEach(rec => {
    ...
    const requested = formatRequestedTimestamp(rec.requestedAt || rec.resubmittedAt);

    tr.innerHTML = `
      <td>
        <input type="checkbox" class="pending-select pending-checkbox" value="${escapeHtml(rec.id)}">
      </td>
      <td>${escapeHtml(rec.displayName || 'Unknown')}</td>
      ...
      <td>
        <div class="pending-row-actions">
          <button class="primary btn-small pending-approve-btn" type="button">✅ Approve</button>
          <button class="danger btn-small pending-deny-btn" type="button">❌ Deny</button>
        </div>
      </td>
    `;
  ...
  approveSelectedBtn.addEventListener('click', async () => {
    ...
    if (!confirm(`Are you sure you want to approve ${ids.length} selected request(s)?`)) {
      return;
    }
    ...
    selectAll.disabled = tbody.querySelectorAll('tr.pending-row').length > 0;
    updateBulkButtons();
  });
  ...
  denySelectedBtn.addEventListener('click', async () => {
    ...
    if (!confirm(`Are you sure you want to deny ${ids.length} selected request(s)?`)) {
      return;
    }
  ...
  tbody.addEventListener('click', async (e) => {
    ...
    if (btn.classList.contains('pending-approve-btn')) {
      if (!confirm('Approve this proposed clock-out?')) return;
    ...
    if (btn.classList.contains('pending-deny-btn')) {
      if (!confirm('Deny this proposed clock-out? The employee can submit again.')) return;
...
async function processPendingVerification(id, approve, ui) {
  const row = ui?.tbody
    ? Array.from(ui.tbody.querySelectorAll('tr.pending-row')).find(r => r.dataset.id === id)
    : null;
  ...
  try {
    ...
    if (approve) {
      ...
      const alreadyExists = logs.some(log =>
        log?.type === 'out' &&
    ...
    alert(`Failed to ${approve ? 'approve' : 'deny'} this request.`);
